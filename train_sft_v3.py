...
train_sft_v3.py — Enhanced SFT training over v1:
  - 200 seeds x 3 difficulties (easy/medium/hard) x 3 families => ~1800 trajectories
  - Augmentation: sometimes insert VERIFY_FLAG before FLIP_FLAG to teach name-copying
  - Qwen3-4B full fine-tune default (fits on 5090 with batch=2)
  - Early stopping on eval_loss plateau (patience=3)
  - load_best_model_at_end so we keep the best checkpoint, not the last one
"""
...
def plan_for_scenario(env, augment=False):
    """Read issue.required_action / valid_actions / blocking_action / valid_resolutions
    directly from procedural scenarios. If augment=True, sometimes insert VERIFY_FLAG
    before FLIP_FLAG so the model learns to copy the exact flag name from feedback."""
    issues = env._issues
    ...
    def emit(cmd):
        ...
        # Augmentation: 50% of FLIP_FLAGs get preceded by VERIFY_FLAG <same_name>
        if augment and verb == "FLIP_FLAG" and random.random() < 0.5:
            parts = cmd.split()
            if len(parts) >= 2:
                flag_name = parts[1]
                if current_desk != "RELEASE":
                    plan.append("SWITCH_DESK RELEASE")
                    current_desk = "RELEASE"
                plan.append(f"VERIFY_FLAG {flag_name}")
...
def build_dataset(seeds_per_family=200, difficulties=("easy", "medium", "hard")):
    all_examples = []
    kept = 0
    dropped = 0

    # Hand-authored scenarios, both clean and augmented
    ...
    # Procedural: many seeds x all difficulties x both augment settings
...
def main():
    ...
    random.seed(42)
    all_examples = build_dataset(seeds_per_family=args.seeds)
    print()

    if len(all_examples) < 3000:
        print(f"WARNING: only {len(all_examples)} examples. Expected ~10000 at seeds=200.")
